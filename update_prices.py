...
# ── iShares Physical Silver vía Yahoo Finance (ISLN.L — USD London → EUR) ───────
try:
    r_silver = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/ISLN.L?interval=1d&range=1d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    price_silver_usd = r_silver.json()['chart']['result'][0]['meta']['regularMarketPrice']
    eur_usd = prices.get('EURUSD', 1.12)
    price_silver_eur = round(price_silver_usd / eur_usd, 5)
    prices['IE00B4NCWG09'] = price_silver_eur
    print(f"  ✅ iShares Physical Silver: ${price_silver_usd} → €{price_silver_eur}")
except Exception as e:
    print(f"  ❌ iShares Physical Silver: {e}")
...
portfolio = [
    ...
    {'isin': 'IE00B4NCWG09', 'qty': 80},            # iShares Physical Silver
    ...
    {'isin': 'BTC',          'qty': 0.00298483},    # Bitcoin
]
