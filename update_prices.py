...
def yahoo_xetra(symbol, name, reference=None):
    ...
    se descarta el dato si difiere >25% — protege contra datos corruptos/obsoletos."""
    try:
...
print(f"🔄 Actualizando precios Marco v13... (curl_cffi: {'sí' if HAS_CURL_CFFI else 'NO disponible, usando requests'})\n")
...
# ── Bitcoin y Hyperliquid vía CoinGecko (EUR) ─────────────────────────────────
try:
    r = requests.get(
        'https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,hyperliquid&vs_currencies=eur',
        timeout=10)
    data = r.json()
...
# ── Cálculo total cartera Marco (v13 — sin ETH, con ASST 21/09/2026) ──────────
portfolio = [
    {'isin': 'IE000ZYRH0Q7', 'qty': 96.6},           # iShares Developed World ✅ 21/09
    {'isin': 'IE000QAZP7L2', 'qty': 29.71},          # iShares Emerging Markets ✅ 21/09
    {'isin': 'IE00B43VDT70', 'qty': 60},             # Invesco Physical Silver ✅ 21/09
    {'isin': 'IE000UL6CLP7', 'qty': 272},            # Global X Silver Miners
    {'isin': 'ES0146309002', 'qty': 4.07954596},     # Horos Value Internacional ✅ 21/09
    {'isin': 'ES0175902008', 'qty': 47.493437},      # Sigma Internacional ✅ 21/09
    {'isin': 'ES0112611001', 'qty': 14.525463},      # Azvalor Internacional FI
    {'isin': 'US88160R1014', 'qty': 0.543552},       # Tesla (Xetra EUR)
    {'isin': 'ES0126962069', 'qty': 15000},          # Nueva Expresion Textil ✅ 21/09
    {'isin': 'BTC',          'qty': 0.06075519},     # Bitcoin ✅ 21/09
    {'isin': 'ASST',         'qty': 35},             # ASST (Strive) ✅ 21/09
    {'isin': 'HYPE',         'qty': 7.93650794},     # Hyperliquid ✅ 24/08
    {'isin': 'IE00B4ND3602', 'qty': 3},              # iShares Physical Gold ✅ 21/09
]

fallback = {
    'IE000ZYRH0Q7': 12.071,   'IE000QAZP7L2': 13.554,
    'IE00B43VDT70': 55.07,    'IE000UL6CLP7': 38.90,
    'ES0112611001': 337.44291, 'US88160R1014': 356.60,
    'ES0126962069': 1.066,    'BTC': 56524,
    'ASST': 26.30,            'HYPE': 69.963,
    'IE00B4ND3602': 73.59,
    'ES0146309002': 217.2901, 'ES0175902008': 20.30,
}
...
print(f"\n📊 Total Marco ({today}): €{total:,.2f} ({len(prices)}/14 precios)")
