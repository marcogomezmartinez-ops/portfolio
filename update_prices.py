...
# ── iShares Physical Silver vía Yahoo Finance (PPFS.SG — EUR, Stuttgart) ───────
try:
    r_silver = requests.get('https://query1.finance.yahoo.com/v8/finance/chart/PPFS.SG?interval=1d&range=1d',
                     headers={'User-Agent': 'Mozilla/5.0'}, timeout=15)
    price_silver_eur = r_silver.json()['chart']['result'][0]['meta']['regularMarketPrice']
    prices['IE00B4NCWG09'] = round(price_silver_eur, 5)
    print(f"  ✅ iShares Physical Silver: €{round(price_silver_eur, 5)}")
except Exception as e:
